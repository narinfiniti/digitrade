...
public sealed class RiskOutboxStore(RiskDbContext dbContext) : IRiskOutboxWriter, IOutboxStore
{
    public async Task EnqueueAsync(IReadOnlyCollection<IDomainEvent> domainEvents, CancellationToken cancellationToken = default)
    {
        ...
        var entities = domainEvents
            .Select(RiskOutboxMessageFactory.Create)
            .Select(ToEntity)
            .ToArray();
    ...
    public async Task MarkFailedAsync(Guid messageId, string failureReason, DateTimeOffset failedAtUtc, CancellationToken cancellationToken = default)
    {
        ...
        message.FailureReason = string.IsNullOrWhiteSpace(failureReason)
            ? "Risk outbox publish failed."
            : failureReason;

        await dbContext.SaveChangesAsync(cancellationToken);
    }

    private static RiskOutboxMessageEntity ToEntity(OutboxMessage message)
    {
        return new RiskOutboxMessageEntity
        {
    ...
    private static OutboxMessage ToModel(RiskOutboxMessageEntity message)
    {
