...
public sealed class RiskOutboxPublisher(
    IOutboxStore outboxStore,
    IIntegrationEventPublisher integrationEventPublisher,
    ILogger<RiskOutboxPublisher> logger,
    TimeProvider timeProvider) : IRiskOutboxPublisher
{
    ...
    private static readonly Action<ILogger, Guid, string, Exception?> PublishFailureLog =
        LoggerMessage.Define<Guid, string>(
            LogLevel.Error,
            new EventId(1, nameof(RiskOutboxPublisher)),
            "Risk outbox publish failed for message {MessageId} and event {EventName}.");
    ...
    public async Task PublishPendingAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var pendingMessage in pendingMessages)
        {
            try
            {
                var envelope = RiskOutboxEnvelopeFactory.Create(pendingMessage);
