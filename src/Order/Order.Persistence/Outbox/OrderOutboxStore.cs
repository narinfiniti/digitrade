...
public sealed class OrderOutboxStore(OrderDbContext dbContext) : IOrderOutboxWriter, IOutboxStore
{
    public async Task EnqueueAsync(IReadOnlyCollection<IDomainEvent> domainEvents, CancellationToken cancellationToken = default)
    {
        ...
        var entities = domainEvents
            .Select(OrderOutboxMessageFactory.Create)
            .Select(ToEntity)
            .ToArray();
    ...
    private static OrderOutboxMessageEntity ToEntity(OutboxMessage message)
    {
        return new OrderOutboxMessageEntity
        {
    ...
    private static OutboxMessage ToModel(OrderOutboxMessageEntity message)
    {
